...
VIPSTARCOIN.DEFAULT_TIP_WAIT_TIME = 5000;
...
VIPSTARCOIN.prototype._initDefaults = function(options) {
  ...
  // wait tip
  this.tipWaitTime = options.tipWaitTime || VIPSTARCOIN.DEFAULT_TIP_WAIT_TIME;
};
...
VIPSTARCOIN.prototype.subscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'vipstarcoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
...
VIPSTARCOIN.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'vipstarcoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
...
VIPSTARCOIN.prototype._initChain = function(callback) {
  ...
  async.waterfall([function (cb) {
    return self.client.getBestBlockHash(function (err, response) {
      if (err) {
    ...
    return self.client.getBlock(blockHash, function (err, response) {
      if (err) {
        return cb(self._wrapRPCError(err));
      }
    ...
    return self.client.getBlockHash(0, function(err, response) {
      if (err) {
        return cb(self._wrapRPCError(err));
      }
      genesisBlockHash = response.result;
    });
  }, function (cb) {
    return self.getRawBlock(genesisBlockHash, function(err, blockBuffer) {
      if (err) {
        return cb(err);
      }
    ...
    return self.getInfo(function (err, info) {
      if (err) {
        return cb(err);
      }

      self.moneysupply = info.moneysupply;
    })
  }], function (err) {
    if (err) {
      return callback(err);
...
VIPSTARCOIN.prototype._updateTip = function(node, message) {
  ...
  if (hex !== self.tiphash) {
    ...
    node.client.getInfo(function (err, result) {
      if (err) {
        var error = self._wrapRPCError(err);
        self.emit('error', error);
      } else {
        self.moneysupply = result.moneysupply;
    ...
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('vipstarcoind/addressbalance', {
                    address: address,
                    txid: txid,
